...
//  }
//
////    console.log('minifyJSData >>>>>', ugly.map);
//  this.data = ugly.code;
//};
...
function aggregationRequestHandler(meanioinstance, req, res, next){
  ...
  meanioinstance.aggregatedsrc('js', 'footer', onAggregatedSrc.bind(null,'footer','js',res,null));
  meanioinstance.aggregatedsrc('map', 'header', onAggregatedSrc.bind(null,'header','map',res,null));
  meanioinstance.aggregatedsrc('map', 'footer', onAggregatedSrc.bind(null,'footer','map',res,next));
}
...
function minifyJSData(stringData){
    var ugly;
    try {
        ugly = uglify.minify(stringData, {
            fromString: true,
            mangle: false,
            outSourceMap: "/modules/aggregated.map"
        });
    }
    catch (e) {
        console.log('\\n\nError in', stringdata, 'on line', e.line);
...
function supportAggregate(Meanio) {
  ...
  Meanio.prototype.aggregated = function(ext, group, callback) {
    //  \\ Data already exists and is ready
    ...
    var mapLocation = '\n//# sourceMappingURL=/modules/aggregated.map';
    if(ext==='js'){
        if (!this.minify) {
            this.minify = {};
        }
        if (!this.minify[group]) {
            this.minify[group] = minifyJSData(aggregated.js[group].getData());
        }
      callback(this.minify[group].code);
      return;
    }
    if(ext==='map'){
        if (!this.minify) {
      ...
      return callback(this.minify[group].map);
      return;
    }
    callback('');
  };
